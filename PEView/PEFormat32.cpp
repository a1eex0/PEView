...
// 此代码模块中包含的函数的前向声明:
VOID InitTreeView32(PCHAR buffer, LPWSTR lpFileName);
VOID InitListView32(PCHAR buffer, DWORD dwStart, DWORD dwReadLength);


//
//  函数: FormatMain32(HWND hWnd, PCHAR buffer, LPWSTR lpFilePath)
//
//  目标: PE32文件处理主程序
//
VOID FormatMain32(HWND hWnd, PCHAR buffer, LPWSTR lpFilePath)
{
	InitTreeView32(buffer, szFileName);
	/*InitListView32(buffer,0,dwFileLength);*/
	SetStatusText(hwndStatus, szFileName);
}

//
//  函数: InitTreeView32(PCHAR buffer, LPWSTR lpFileName)
//
//  目标: 设置树视图显示内容
//
VOID InitTreeView32(PCHAR buffer, LPWSTR lpFileName)
{
	...
	// 添加 NT 头节点
	hNTHeaders = AddItemToTree(hwndTreeView, L"IMAGE_NT_HEADERS32", hRoot, TRUE);
	// 为 NT 头添加子节点
	...
	hNTOptionalHeader = AddItemToTree(hwndTreeView, L"IMAGE_OPTIONAL_HEADER32", hNTHeaders, FALSE);
	// 添加 Section 节点
	hSectionHeaders = AddItemToTree(hwndTreeView, L"IMAGE_SECTION_HEADERS", hRoot, TRUE);

	PIMAGE_DOS_HEADER pDos = (PIMAGE_DOS_HEADER)buffer;
	PIMAGE_NT_HEADERS32 pNt = (PIMAGE_NT_HEADERS32)(pDos->e_lfanew + buffer);
...
//
//  函数: InitListView32(PCHAR buffer, DWORD dwStart, DWORD dwReadLength)
//
//  目标: 设置列表视图显示文件十六进制内容
//
VOID InitListView32(PCHAR buffer, DWORD dwStart, DWORD dwReadLength)
...
//
//  函数: CLICK_IMAGE_DOS_HEADER(PCHAR buffer)
//
//  目标: DOS 头列表显示函数
//
VOID CLICK_IMAGE_DOS_HEADER(PCHAR buffer)
{
	WCHAR strBuffer[9];
	// 添加分列
...
//
//  函数: CLICK_IMAGE_NT_HEADERS32(PCHAR buffer)
//
//  目标: NT 头十六进制列表显示函数
//
VOID CLICK_IMAGE_NT_HEADERS32(PCHAR buffer)
{
	PIMAGE_DOS_HEADER pDos = (PIMAGE_DOS_HEADER)buffer;
	PIMAGE_NT_HEADERS32 pNt = (PIMAGE_NT_HEADERS32)(pDos->e_lfanew + buffer);
...
//
//  函数: CLICK_NT_Signature(PCHAR buffer)
//
//  目标: NT 头签名列表显示函数
//
VOID CLICK_NT_Signature(PCHAR buffer)
{
	WCHAR strBuffer[9];
	// 添加分列
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
...
//
//  函数: CLICK_IMAGE_FILE_HEADER(PCHAR buffer)
//
//  目标: 文件头列表显示函数
//
VOID CLICK_IMAGE_FILE_HEADER(PCHAR buffer)
{
	WCHAR strBuffer[27];
	// 添加分列
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	...
	case IMAGE_FILE_MACHINE_I386:
		wcscpy_s(strBuffer, L"Intel 386.");
		break;
	default:
...
//
//  函数: CLICK_IMAGE_OPTIONAL_HEADER32(PCHAR buffer)
//
//  目标: 文件头列表显示函数
//
VOID CLICK_IMAGE_OPTIONAL_HEADER32(PCHAR buffer)
{
	WCHAR strBuffer[27];
	// 添加分列
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	...
	ListView_SetItemText(hwndListView, line, 3, L"PE32");
...
//
//  函数: CLICK_IMAGE_SECTION_HEADERS(PCHAR buffer)
//
//  目标: 区块十六进制列表显示函数
//
VOID CLICK_IMAGE_SECTION_HEADERS(PCHAR buffer) 
{
	PIMAGE_DOS_HEADER pDos = (PIMAGE_DOS_HEADER)buffer;
	PIMAGE_NT_HEADERS32 pNt = (PIMAGE_NT_HEADERS32)(pDos->e_lfanew + buffer);
...
//
//  函数: CLICK_IMAGE_EXPORT_DIRECTORY(PCHAR buffer)
//
//  目标: 导出表列表显示函数
//
VOID CLICK_IMAGE_EXPORT_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	AddListViewColumn(hwndListView, 1, 1, L"Data", 100);
	...
	for (size_t i = 0; i <= pExport->NumberOfFunctions; i++)
	{
		...
		for (size_t n = 0; n < pExport->NumberOfNames; n++)
		{
			if ((pID[n] + pExport->Base) == (i + 1))
			{
				...
				ListView_SetItemText(hwndListView, line, 1, strBuffer);
				_stprintf_s(strBuffer, L"%08X", pEAT32[pID[n]]);
				ListView_SetItemText(hwndListView, line, 2, strBuffer);
				PCHAR szFuncName = (PCHAR)(RvaToOffset(pENT32[n], buffer) + buffer);
				memset(strName, 0, sizeof(strName));
...
//
//  函数: CLICK_IMAGE_IMPORT_DIRECTORY(PCHAR buffer)
//
//  目标: 导入目录列表显示函数
//
VOID CLICK_IMAGE_IMPORT_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	AddListViewColumn(hwndListView, 1, 1, L"Data", 100);
	AddListViewColumn(hwndListView, 0, 2, L"Description", 225);
...
//
//  函数: CLICK_IMAGE_RESOURCE_DIRECTORY(PCHAR buffer)
//
//  目标: 资源目录列表显示函数
//
VOID CLICK_IMAGE_RESOURCE_DIRECTORY(PCHAR buffer)
{
...
//
//  函数: CLICK_IMAGE_EXCEPTION_DIRECTORY(PCHAR buffer)
//
//  目标: 异常目录列表显示函数
//
VOID CLICK_IMAGE_EXCEPTION_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
...
//
//  函数: CLICK_IMAGE_CERTIFICATE_DIRECTORY(PCHAR buffer)
//
//  目标: 证书目录列表显示函数
//
VOID CLICK_IMAGE_CERTIFICATE_DIRECTORY(PCHAR buffer)
{
...
//
//  函数: CLICK_IMAGE_BASE_RELOC_DIRECTORY(PCHAR buffer)
//
//  目标: 重定位目录列表显示函数
//
VOID CLICK_IMAGE_BASE_RELOC_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	AddListViewColumn(hwndListView, 1, 1, L"Data", 100);
...
//
//  函数: CLICK_IMAGE_DEBUG_DIRECTORY(PCHAR buffer)
//
//  目标: 调试目录列表显示函数
//
VOID CLICK_IMAGE_DEBUG_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	AddListViewColumn(hwndListView, 1, 1, L"Data", 100);
	AddListViewColumn(hwndListView, 0, 2, L"Description", 200);
...
//
//  函数: CLICK_IMAGE_TLS_DIRECTORY(PCHAR buffer)
//
//  目标: TLS目录列表显示函数
//
VOID CLICK_IMAGE_TLS_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	AddListViewColumn(hwndListView, 1, 1, L"Data", 100);
	AddListViewColumn(hwndListView, 0, 2, L"Description", 200);
	...
	PIMAGE_TLS_DIRECTORY32 pTLS = (PIMAGE_TLS_DIRECTORY32)(RvaToOffset(pTLSDir->VirtualAddress, buffer) + buffer);
...
//
//  函数: CLICK_IMAGE_LOAD_CONFIG_DIRECTORY(PCHAR buffer)
//
//  目标: 加载配置目录列表显示函数
//
VOID CLICK_IMAGE_LOAD_CONFIG_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	AddListViewColumn(hwndListView, 1, 1, L"Data", 100);
	...
	PIMAGE_LOAD_CONFIG_DIRECTORY32 pLoadConfig = (PIMAGE_LOAD_CONFIG_DIRECTORY32)(RvaToOffset(pLoadConfigDir->VirtualAddress, buffer) + buffer);
...
//
//  函数: CLICK_IMAGE_BOUND_IMPORT_DIRECTORY(PCHAR buffer)
//
//  目标: 绑定导入目录列表显示函数
//
VOID CLICK_IMAGE_BOUND_IMPORT_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	AddListViewColumn(hwndListView, 1, 1, L"Data", 100);
	AddListViewColumn(hwndListView, 0, 2, L"Description", 200);
...
//
//  函数: CLICK_IMAGE_IAT_DIRECTORY(PCHAR buffer)
//
//  目标: 导入地址表列表显示函数
//
VOID CLICK_IMAGE_IAT_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	...
	while (pImport->Name != NULL)
	{
		...
		PIMAGE_THUNK_DATA32 pIAT = (PIMAGE_THUNK_DATA32)(RvaToOffset(pImport->OriginalFirstThunk, buffer) + buffer);
...
//
//  函数: CLICK_IMAGE_DELAY_IMPORT_DIRECTORY(PCHAR buffer, LPWSTR szText)
//
//  目标: 延迟加载导入目录列表显示函数
//
VOID CLICK_IMAGE_DELAY_IMPORT_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	AddListViewColumn(hwndListView, 1, 1, L"Data", 100);
	AddListViewColumn(hwndListView, 0, 2, L"Description", 200);
...
//
//  函数: CLICK_IMAGE_COM_DESCRIPTOR_DIRECTORY(PCHAR buffer, LPWSTR szText)
//
//  目标: COM 运行目录列表显示函数
//
VOID CLICK_IMAGE_COM_DESCRIPTOR_DIRECTORY(PCHAR buffer)
{
	AddListViewColumn(hwndListView, 0, 0, L"pFile", 75);
	AddListViewColumn(hwndListView, 1, 1, L"Data", 100);
	AddListViewColumn(hwndListView, 0, 2, L"Description", 200);
...
//
//  函数: CLICK_SECTION_LIST(PCHAR buffer, LPWSTR szText)
//
//  目标: 区块项点击列表显示函数
//
VOID CLICK_SECTION_LIST(PCHAR buffer, LPWSTR szText)
{
...
//
//  函数: TreeToList32(PCHAR buffer, LPWSTR szText, LPWSTR lpFilePath)
//
//  目标: 树视图子节点列表视图响应
//
